        ...
        figure_selection = pd.DataFrame(
            [
                ...
                (
                    ...
                    "Checks model-assisted OPE signal quality.",
                    True,
...
class RawToPaperPipeline:
    ...
    def prune_to_paper_artifacts(self) -> None:
        """Keep only artifacts needed for manuscript figures, tables, checks, and summaries."""

        metadata_keep = {
            "advanced_policy_conservative_ranking.csv",
            "advanced_policy_heterogeneity.csv",
            "conservative_ranking_comparison.csv",
            "decision_rule_ablation_summary.csv",
            "decision_rule_bootstrap_selection.csv",
            "decision_rule_gate_matrix.csv",
            "equilibrium_sensitivity.csv",
            "final_figure_selection.csv",
            "final_policy_recommendation.csv",
            "final_table_selection.csv",
            "final_validation_sequence.csv",
            "launch_readiness_checklist.csv",
            "marketplace_scorecard.csv",
            ...
            "paper_table_season3_transfer.csv",
            "paper_table_theory_empirical_link.csv",
            "prototype_calibration_summary.csv",
            "reserve_policy_daily_effects.csv",
            "season2_outcome_density.csv",
            "season2_vs_season3_policy_transfer.csv",
            "season3_priority_policy_daily_validation.csv",
            "season3_priority_policy_validation.csv",
            "support_collapse_curve.csv",
            "theory_guided_sensitivity_verdict.csv",
            "validation_design_detectability.csv",
        }
        table_keep = {
            "paper_table_data_windows.csv",
            "paper_table_decision_ablation.csv",
        ...
        processed_keep = {"ipinyou_nuisance_prototype_opportunity_panel.parquet"}
        removed = 0
        for folder, keep in [
            (self.workspace.metadata_dir, metadata_keep),
            (self.workspace.table_dir, table_keep),
            (self.workspace.processed_dir, processed_keep),
        ]:
            for path in sorted(folder.rglob("*"), reverse=True):
                if path.is_file() and path.name not in keep:
                    ...
                    path.rmdir()
        self.progress.log(f"Pruned non-paper intermediate artifacts; removed {removed:,} file(s).")
